		...
		return {
			...
			posts: user.edge_owner_to_timeline_media.edges.map((edge: any) => {
				...
				return {
					...
					children: edge.node.edge_sidecar_to_children ? edge.node.edge_sidecar_to_children.edges.map((edge: any) => {
						return {
							id: edge.node.id,
							shortCode: edge.node.shortcode,
							dimensions: edge.node.dimensions,
...
const getPostData = async (shortcode: string) => {
	...
	if (GQL) {
		...
		return {
			...
			childrenPictures: media_data.edge_sidecar_to_children ? media_data.edge_sidecar_to_children.map((edge: any) => {
				return {
					id: edge.node.id,
					shortcode: edge.node.shortcode,
					dimensions: edge.node.dimensions,
			...
			taggedUsers: (media_data.edge_media_to_tagged_user.edges) ? media_data.edge_media_to_tagged_user.edges.map((tag: any) => {
				return {
